...
class AccountMove(models.Model):
    ...
    product_account_payable_summary = fields.Text(
        string="Account Payable Summary",
        compute="_compute_product_account_payable_summary",
    )
    ...
    def _compute_product_account_payable_summary(self):
        for move in self:
            if not move.is_purchase_document(include_receipts=True):
                move.product_account_payable_summary = False
                continue
            amounts = move._get_account_payable_amounts_by_product()
            if not amounts:
                move.product_account_payable_summary = False
                continue
            ...
            move.product_account_payable_summary = "\n".join(parts)

    def _get_account_payable_amounts_by_product(self):
        self.ensure_one()
        ...
        for line in product_lines:
            account = line._get_account_payable_for_line()
            if not account:
                continue
    ...
    def _should_split_account_payable_by_product(self):
        self.ensure_one()
        amounts = self._get_account_payable_amounts_by_product()
        if len(amounts) > 1:
            return True
        if len(amounts) == 1:
            ...
            partner_account = self.with_company(
                self.company_id
            ).commercial_partner_id.property_account_payable_id
            return bool(partner_account and partner_account.id != account_id)
        return False

    def _build_product_account_payable_needed_terms(self):
        self.ensure_one()
        AccountTax = self.env["account.tax"]
        amounts_by_account = self._get_account_payable_amounts_by_product()
        needed_terms = {}
            ...
            if self.invoice_payment_term_id:
                ...
                for term_line in invoice_payment_terms["line_ids"]:
                    key = frozendict({
                        ...
                        "account_payable_split_id": account_id,
                    })
                    values = {
    ...
    def _compute_needed_terms(self):
        ...
        for invoice in self.filtered(lambda move: move.is_purchase_document(include_receipts=True)):
            if not invoice._should_split_account_payable_by_product():
                continue
            invoice.needed_terms = invoice._build_product_account_payable_needed_terms()
            invoice.needed_terms_dirty = True

    def _get_product_account_payable_summary_for_report(self):
        self.ensure_one()
        amounts = self._get_account_payable_amounts_by_product()
        account_model = self.env["account.account"]
        summary = []
