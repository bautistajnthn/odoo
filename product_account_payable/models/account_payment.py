...
class AccountPayment(models.Model):
    ...
    def _get_bills_for_product_ap_split(self):
        """Return bills whose payable residuals should drive payment AP splits."""
        self.ensure_one()
        bills = self.invoice_ids | self.reconciled_invoice_ids
        if "paid_invoice_ids" in self._fields and self.paid_invoice_ids:
            bills |= self.paid_invoice_ids
        if bills:
        ...
        # Payment register sets destination to a specific bill AP account.
        # Do not rediscover open bills (would wrongly reallocate across accounts).
        partner_ap = self.partner_id.with_company(
            self.company_id
        ...
        if (
            ...
            and self.destination_account_id != partner_ap
        ):
    ...
    def _get_payable_residuals_for_product_ap_split(self):
        """FIFO payable lines: list of (account, residual_currency, residual_balance)."""
        self.ensure_one()
        bills = self._get_bills_for_product_ap_split()
        if not bills:
            return []

        residuals = []
        for bill in bills.sorted(
            key=lambda move: (
            ...
            ).sorted(key=lambda line: (line.date_maturity or bill.invoice_date_due or line.date, line.id))
    ...
    def _get_payment_ap_amounts_by_account(self, amount_currency, balance):
        """Allocate payment counterpart amounts across product AP accounts (FIFO).
        ...
        """
        self.ensure_one()
        residuals = self._get_payable_residuals_for_product_ap_split()
        if not residuals:
            return []
    ...
    def _should_split_payment_ap_by_product(self, amount_currency=None, balance=None):
        self.ensure_one()
        if self.partner_type != "supplier" or self.payment_type != "outbound":
            return False
        if amount_currency is None or balance is None:
            # Approximate signs for outbound: counterpart is debit (positive).
        ...
        amounts = self._get_payment_ap_amounts_by_account(amount_currency, balance)
        if len(amounts) > 1:
            return True
        if len(amounts) == 1:
    ...
    def _prepare_move_line_default_vals(self, write_off_line_vals=None, force_balance=None):
        ...
        splits = self._get_payment_ap_amounts_by_account(
        ...
        for account_id, amount_currency, balance in splits:
            # Fully allocated to product APs: do not keep a zero partner AP line.
    ...
    def _synchronize_to_moves(self, changed_fields):
        """Support multiple AP counterpart lines when splitting by product account."""
        if not any(
        ...
        for pay in self:
            ...
            if (
                ...
                or pay._should_split_payment_ap_by_product()
            ):
                multi_payments |= pay
                line_ids_commands = [
                ...
                pay._remove_zero_amount_payable_lines()

        remaining = self - multi_payments
        if remaining:
            super(AccountPayment, remaining)._synchronize_to_moves(changed_fields)

    def _remove_zero_amount_payable_lines(self):
        """Drop AP counterpart lines with no amount (e.g. unused partner AP)."""
        for pay in self:
            move = pay.move_id
    ...
    def _generate_journal_entry(self, write_off_line_vals=None, force_balance=None, line_ids=None):
        ...
        self._remove_zero_amount_payable_lines()
        return res
